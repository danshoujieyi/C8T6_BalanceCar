...
{
    // 使能GPIO的时钟
    __HAL_RCC_GPIOB_CLK_ENABLE(); // 假设SDA和SCL引脚都在GPIOB上
...
char MPU6050_WriteReg(uint8_t addr,uint8_t regaddr,uint8_t num,uint8_t *regdata)
{
    uint16_t i = 0;
        IIC_Start();
        Send_Byte((addr<<1)|0);
        if( IIC_WaitAck() == 1 ) {IIC_Stop();return 1;}
        Send_Byte(regaddr);
        if( IIC_WaitAck() == 1 ) {IIC_Stop();return 2;}
    
        for(i=0;i<num;i++)
    {
        Send_Byte(regdata[i]);
        if( IIC_WaitAck() == 1 ) {IIC_Stop();return (3+i);}
    }        
        IIC_Stop();
    return 0;
}
...
char MPU6050_ReadData(uint8_t addr, uint8_t regaddr,uint8_t num,uint8_t* Read)
{
        ...
        IIC_Start();
        Send_Byte((addr<<1)|1);
        if( IIC_WaitAck() == 1 ) {IIC_Stop();return 3;}
